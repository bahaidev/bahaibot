...
| `Discord` | `.EmbedBuilder()` | Creates an object and passed to `message.channel.send` | Bahá'í Info, Bahá'í Writings (`quoteBook` and `readRandom` commands)
| `EmbedBuilder()` | `.setAuthor(text, avatarURL)` | | Bahá'í Info, Bahá'í Writings (`quoteBook` and `readRandom` commands)
| `EmbedBuilder()` | `.setDescription(text)` | | Bahá'í Info, Bahá'í Writings (`quoteBook` and `readRandom` commands)
| `EmbedBuilder()` | `.addField('Support Server', markdownLink)` or `addField('Notes', text)` | | Bahá'í Info, Bahá'í Writings (`quoteBook` and `readRandom` commands)
| `EmbedBuilder()` | `.setColor(numberForColorBorder)` | | Bahá'í Writings (`quoteBook` and `readRandom` commands)
...
| `client.user` | `.avatarURL()` | | "Bahá'í Writings" (`quoteBook` and `readRandom` commands)
...
|--------------------------------------------|-------|-------------|
| `content` | String | Accessed by default command, Bahá'í Writings (`quoteBook` command), Admin (`speak`, `echo`, `puppet` commands); Social Info (`seen` command); Bahá'í Wikis (`bp`, `today`, `b9`, `bm` commands)
| `author.id` | String | Accessed by default command; Admin (`speak`, `puppet`, `echo`, `checkin` command)
| `author.username` | String | Accessed by Help (`help` command), Bahá'í Writings (`quoteList` command); Admin (`puppet`, `echo`, and `checkin` commands); Bahá'í Salutations (`abha` command); Salutations (`sup`, `morning`, `afternoon`, `evening`, `hello`, `welcome` commands); Social Info (`users` and `seen` commands); Bahá'í Wikis (`bp`, `today`, `b9`, `bm` commands)
| `channel.send(string)` | See also the object signature | Called by default command (and possibly in dialogflow router); Bahá'í Writings (`quoteBook` and default `reader` fallback commands); Admin (`echo`, `puppet` command); Bahá'í Info (`badi` command); Bahá'í Salutations (`abha`, `nawruz`, `ridvan`, and `ninePointedStar` commands); Salutations (`sup`, `morning`, `afternoon`, `evening`, `hello`, `welcome` commands); Light-hearted (`coffee`, `tea`, `unladen`, `bruh`, `goodbot`, `badbot`, `repeating`, `santacat`, `ping` commands); Social Info (`users` and `seen` commands)
| `channel.send(object)` | Like the `channel.send` above | Bahá'í Writings (`quoteBook` and `readRandom` commands); possibly in dialogflow router (via default command)
| `channel.send(object)` | Like the `channel.send` above though also has `fields` on `embed` which is an array of `{name: string, value: string}` objects (the command and help text for it) | Help (`help` command), Bahá'í Writings (`quoteList` command); Bahá'í Info (`info` command)
