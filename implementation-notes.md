...
| `Discord` | `.EmbedBuilder()` | Creates an object and passed to `message.channel.send` | Bahá'í Info, Bahá'í Writings (`readBook` and `readRandom` commands)
| `EmbedBuilder()` | `.setAuthor(text, avatarURL)` | | Bahá'í Info, Bahá'í Writings (`readBook` and `readRandom` commands)
| `EmbedBuilder()` | `.setDescription(text)` | | Bahá'í Info, Bahá'í Writings (`readBook` and `readRandom` commands)
| `EmbedBuilder()` | `.addField('Support Server', markdownLink)` or `addField('Notes', text)` | | Bahá'í Info, Bahá'í Writings (`readBook` and `readRandom` commands)
| `EmbedBuilder()` | `.setColor(numberForColorBorder)` | | Bahá'í Writings (`readBook` and `readRandom` commands)
